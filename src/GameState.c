...
void afficher(GameState* state) { //réalise un affichage graphique dans le terminal en couleurs
    ...
    for (int y = 0; y < state->size; y++) {
        for (int x = 0; x < state->size; x++) { //On parcourt le plateau
            ...
            if(c >= 1 && c < 10) { //si elle figure parmi les valeurs possibles
...
int entree(int joueur) { //fonction permettant d'entrer des coups. B retourne 1 par exemple
    ...
    while (val == -1) {
        printf("\nJoueur %d, entrez une lettre (A-G): ", joueur == PLAYER_1 ? 1 : 2); //condition ? valeur si vrai : valeur si faux
        scanf(" %c", &x); //On attribut à x le char rentré par l'utilisateur
        if (x >= 'A' && x <= 'G') { //Si x figure parmis les entrées valide (entre A et G)
            val = (x - 'A');//attribut à val la valeur correspondante de x (les valeurs asci des lettres commencent à 65, donc nécéssaire de faire une soustraction, on peut pas juste convertir)
        } 
        else {
...
int choisir_mode() {
    ...
    while(1) {
        ...
        printf("Randy (aleatoire)\nRandy_Jr (aleatoire ameliore)\nBouboule (glouton)\nHegemonique\nHegemonique_Jr (mix hegemonique-glouton)\nBoyard (case la plus lointaine)\nVersus (bot contre bot)\nSuper versus (versus 500 fois)\nJcJ\n\n");//liste des modes possibles
        ...
        if(strcmp(mode,"JcJ")==0) return 2;//utilisateur contre utilisateur
        ...
        if(strcmp(mode,"Hegemonique")==0) return 6;//le plus de frontières en un coup
        if(strcmp(mode,"Boyard")==0) return 7;//La case la plus lointaine possible
        if(strcmp(mode,"Hegemonique_Jr")==0) return 8;//le plus de frontières en un coups, et le plus de case possibles lorque plus aucun coup ne fait augmenter les frontières
        

        

        printf("Veuillez selectionner un mode de jeu valide\n");//si la commande ne figure pas parmi les options données
    }
}
...
int choisir_bot() {//si le mode versus est selectionné. Meme principe que choisir_mode()
    ...
    while(1) {
        ...
        if(strcmp(mode,"Hegemonique")==0) return 3;
        if(strcmp(mode,"Boyard")==0) return 4;
        if(strcmp(mode,"Hegemonique_Jr")==0) return 5;
...
int Bouboule(GameState* state, int joueur)
{
    ...
    for(int color = 0; color < 7; color++)//pour chaque couleur
    {
        ...
        { 
            ...
            int j = to_change[processed].y;//On prends les coordonnées de chaque élément de la liste
...
int jouer_jeu(GameState* state)
{
    ...
    int supround= 500; //nombre de parties à faire à la suite dans le mode versus
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) { //erreur si l'utilisateur ne rentre pas la taille de la carte en argument
        fprintf(stderr, "Utilisation de la commande : %s <taille_carte> (optionnel : --ia <nom_ia> <numero_joueur> <liste_etats>\n", argv[0]);
        return -1;
    }

    int size = atoi(argv[1]); //on convertit la taille entrée en entier
    if (size <= 0) 
    {
        fprintf(stderr, "Erreur : la taille doit etre un entier positif.\n");
        return -1;
    }
    ...
    if (argc >= 6)
    {
        if (argc != 5 + size*size)
        {
            fprintf(stderr, "Erreur : il n'y a pas le bon nombre d'etats pour la taille de grille renseignee.\n");
            return -1;
        }
    }
    ...
    if (argc > 2)
    {
        ...
        return simu; //compris entre 0 et 6
    }
